		...
		<div class="container">
             <div class="col-md-offset-2 col-md-8">
				...
				<?php if(isset($mensagem)){echo $mensagem;} ?>
					...
					if($filtro == "user")
					{ ?>
						<h5><a href="?perfil=artes&p=frm_lista">Sem filtro</a></h5>
					<?php 
					}
					else
					{ ?>
						<h5><a href="?perfil=artes&p=frm_lista&user=<?php echo $_SESSION['idUsuario'] ?>">Mostrar os registros editados por <?php echo $_SESSION['nomeCompleto']; ?></a></h5>
			  <?php } ?>	
					</div>
            </div>
			<div class="table-responsive list_info">
				<table class="table table-condensed"><script type=text/javascript language=JavaScript src=../js/find2.js> </script>
					<thead>
						...
						$sql_lista = "SELECT acervo_registro.titulo,acervo_registro.id_tabela, acervo_registro.id_acervo FROM acervo_registro,acervo_artes WHERE acervo_registro.id_tabela = acervo_artes.id AND acervo_registro.publicado = '1' AND acervo_registro.tabela = '126' $filtro ORDER BY acervo_registro.data_catalogacao DESC";
						...
						while($x = mysqli_fetch_array($limite)){
							$autoridades = retornaAutoridades(idReg($x['id_tabela'],126),NULL,1);
					...
					<td class="list_description">
					<form action="?perfil=artes&p=frm_atualiza" method="post">
...
<input type="submit" class="btn btn-theme btn-block" value='Editar' name='enviar'></form></td>
					<td class="list_description">
					<form action="?perfil=artes&p=frm_lista" method="post">
<input type="hidden" name="idDisco" value="<?php echo $x['id_tabela']?>" />
...
<input type="submit" class="btn btn-theme btn-block" value='Duplicar' name='apagar'></form>-->
		<a href="?perfil=artes&p=frm_resumo&id=<?php echo idReg($x['id_tabela'],$_SESSION['idTabela']); ?>" class="btn btn-theme btn-block" target="_blank">Detalhes</a>
            ...
            <div class="col-md-offset-2 col-md-8">
				...
				$proximo = $pc + 1;
				if($pc > 1){
					echo "<a href='?perfil=artes&p=frm_lista&n_pag=$anterior'><- Anterior</a>";
				}
				echo " | ";
				if($pc < $tp) {
 				 echo " <a href='?perfil=artes&p=frm_lista&n_pag=$proximo'>Próxima -></a>";
 				 }
				?>            
            </div>
          </div>   
