...
//
	...
	$sql = "SELECT * FROM ig_usuario, ig_instituicao, ig_papelusuario WHERE ig_usuario.nomeUsuario = '$usuario' AND ig_usuario.publicado = '1' LIMIT 0,1";
	echo $sql;
	$con = bancoMysqli();
...
function listaModulosAlfa($perfil){ //gera as tds dos módulos a carregar
	...
	$sql_perfil = "SELECT acesso FROM ig_papelusuario WHERE idPapelUsuario = '$perfil'";
	$query_perfil = mysqli_query($con, $sql_perfil);
	$perfil = mysqli_fetch_array($query_perfil);
	
	$x = converterObjParaArray(json_decode($perfil['acesso']));
	$y =  array_keys($x);
	...
	$query_modulos = mysqli_query($con,$sql_modulos);
   	while($modulos = mysqli_fetch_array($query_modulos)){
		...
		if(in_array($modulos['pag'],$y)  AND $x[$modulos['pag']] == 1){
...
function verificaExiste($tabela,$campo,$valor){
	$con = bancoMysqli();
...
function dimensao($string){
	...
	if($string == ""){
		...
		$p = explode("x",strtolower($string));
		$x['altura'] = tiraVirgula($p[0]);
		$x['largura'] = tiraVirgula($p[1]);
		if(isset($p[2])){
			$x['comprimento'] = tiraVirgula($p[2]);
		}
	}		
	return $x;
}
